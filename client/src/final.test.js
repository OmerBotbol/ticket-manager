...
describe(projectName, () => {
  ...
  test('Can change done/undone by clicking a button', async () => {
    await nock('http://localhost:3000/', { allowUnmocked: true })
      .get(`/api/tickets`)
      .query(() => true)
      .reply(200, mockData);

    const doneStatus =  await nock('http://localhost:3000/', { allowUnmocked: true })
      .patch(`/api/tickets/${mockData[0]._id}/done`)
      .query(() => true)
    ...
    await page.hover('.ticket');
    await page.click('.ticket .done-status-button');
    expect(doneStatus.isDone()).toBe(true)
    let isDone = await page.$$('.done');
    expect(isDone.length).toEqual(1);

    const undoneStatus =  await nock('http://localhost:3000/', { allowUnmocked: true })
      .patch(`/api/tickets/${mockData[0]._id}/undone`)
      .query(() => true)
      .reply(200, {updated: true});

    await page.click('.ticket .done-status-button');
    await (new Promise((resolve) => setTimeout(resolve, 1000)));
    expect(undoneStatus.isDone()).toBe(true)
    isDone = await page.$$('.done');
    expect(isDone.length).toEqual(0);
